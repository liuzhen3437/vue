  ...
  <div class="app-container" v-show='$route.meta.showRole'>
    ...
    <div class="table-container">
      <el-tabs v-model="activeName" style="margin-top:15px;" type="border-card">
          ...
          <el-table ref="addressTable"
            ...
            <el-table-column label="名称" align="center">
              <template slot-scope="scope">{{scope.row.name}}</template>
						...
						<el-table-column label="设备用户名" align="center">
            ...
            <el-table-column label="操作" width="300" align="center">
              <template slot-scope="scope">
								<el-button
								  ...
								  @click="handleDelete(scope.$index, scope.row)">设为临时人员
								</el-button>
                <el-button
                  size="mini"
        ...
        <el-tab-pane label="临时人员设置" name="second">
          <el-table ref="tempCardTable"
            ...
            <el-table-column label="操作" width="200" align="center">
              <template slot-scope="scope">
                <el-button
                  size="mini"
                  @click="handleUpdateTempCard(scope.$index, scope.row)">设置为人员
                </el-button>
                <el-button
                  size="mini"
                  type="danger"
...
import {fetchList as fetchListTempCard,deleteBatch as deleteBatchTempCard, deleteTempCard ,changeToInspector,changeToInspectorBatch} from '@/api/tempCard'
  ...
  export default {
    ...
    methods: {
			...
			DotAdd(){
				this.$router.push({path: '/setting/staff/staff/add'})
			},
      getList() {
      ...
      handleUpdate(index, row) {
        this.$router.push({path: '/setting/staff/staff/update', query: {id: row.id}})
      },
      ...
      handleUpdateTempCard(index, row) {
        this.$confirm('是否要设置为人员', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          changeToInspector(row.id).then(response => {
            this.$message({
              message: '设置成功',
              type: 'success',
              duration: 1000
      ...
      handleBatchOperateTempCard() {
        ...
        if (this.operateTypeTempCard === 'deleteBatch') {
        ...
        } else if(this.operateTypeTempCard === 'changeToInspectorBatch'){
          let ids = [];
          for (let i = 0; i < this.multipleSelectionTempCard.length; i++) {
            ids.push(this.multipleSelectionTempCard[i].id);
          }
          let data = new URLSearchParams();
          data.append("ids", ids);
          changeToInspectorBatch(data).then(response => {
            this.$message({
              message: '删除成功',
              type: 'success',
              duration: 1000
