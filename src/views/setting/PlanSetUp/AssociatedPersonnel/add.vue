...
<script>
  import {listUnchecked,lineAddressEventCreateBatch} from '@/api/setting/AssociatedPersonnel'
  export default {
    name: 'lineList',
    data() {
      return {
			...
			binding(index, row){
					this.$confirm('是否要绑定', '提示', {
					  ...
					  type: 'warning'
						...
						data.append("inspectorIds", row.inspectorId);
      ...
      handleBatchOperate() {
        ...
        if (this.operateType === 'addBatch') {
          ...
          for (let i = 0; i < this.multipleSelection.length; i++) {
            inspectorIds.push(this.multipleSelection[i].deviceId);
          }
          let data = new URLSearchParams();
					data.append("planId", this.$route.query.id);
					data.append("inspectorIds", inspectorIds);
					console.log(data)
          lineAddressEventCreateBatch(data).then(response => {
            this.getList();
            this.$message({
