...
<script>
  import {listUnchecked,addressEventCreateBatch} from '@/api/setting/LookCorrelation'
  export default {
    name: 'lineList',
    data() {
      return {
			...
			binding(index, row){
					this.$confirm('是否要绑定', '提示', {
					  ...
					  type: 'warning'
						...
						data.append("addressId", this.$route.query.id);
						data.append("eventIds", row.eventId);
					  addressEventCreateBatch(data).then(response => {
					    this.$message({
					      message: '绑定成功',
					      type: 'success',
					      duration: 1000
      ...
      handleBatchOperate() {
        ...
        if (this.operateType === 'addBatch') {
          ...
          for (let i = 0; i < this.multipleSelection.length; i++) {
            eventIds.push(this.multipleSelection[i].eventId);
          }
          let data = new URLSearchParams();
					data.append("addressId", this.$route.query.id);
