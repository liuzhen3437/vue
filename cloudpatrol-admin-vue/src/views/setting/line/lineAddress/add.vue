...
<script>
  import {listUnchecked,lineAddressEventCreateBatch} from '@/api/setting/lineAddress'
  export default {
    name: 'lineList',
    data() {
      return {
			...
			binding(index, row){
					this.$confirm('是否要绑定', '提示', {
					  ...
					  type: 'warning'
						...
						data.append("lineId", this.$route.query.id);
						data.append("addressIds", row.addressId);
      ...
      handleBatchOperate() {
        ...
        if (this.operateType === 'addBatch') {
          ...
          for (let i = 0; i < this.multipleSelection.length; i++) {
            addressIds.push(this.multipleSelection[i].addressId);
          }
          let data = new URLSearchParams();
					data.append("lineId", this.$route.query.id);
