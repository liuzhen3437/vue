    ...
    methods: {
          ...
          showLoading();
      		...
      		for(var i=this.pageStart;i<=this.pageEnd;i++){
      		  this.rangeDialogVisibleData.pageNum=i;
      		   await listRecordEvent(this.rangeDialogVisibleData).then(response => {
      ...
      exportData(){
             ...
             require.ensure([], () => {
                 ...
                 //要输出的表头
                 const tHeader = ['卡号', '上传时间', '巡检时间', '地点', '事件', '设备', '人员','单位'];
                 //表头对应的内容, 会从下行定义的 list 里去找相应的数据
                 const filterVal = ['card','createDateTime','patrolDateTime','addressName','eventName','deviceName','inspectorName','companyName'];
                 //数据来源
                 const list = this.excelList;
                 const data = this.formatJson(filterVal, list);
                 //fileName: 要导出的表格名称
                 export_json_to_excel(tHeader, data, '事件报表');
